...
**Browse** — a searchable list of shared trips, each showing a cover photo,
location, author, and a place/day count. **View** opens a trip's full
itinerary alongside its **Suggestions** thread — a place to add, a day to
reorder, a heads-up about something that's changed — read and posted right
there without leaving TREK. **Import** pulls a trip in as a local, read-only
draft copy under **My imported trips** on the same page; removing one only
deletes your local copy, nothing on GitHub changes.
